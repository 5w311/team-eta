...
export const PRESETS = {
  Conservative:{mph:64,fuelEvery:500,fuelMin:15,swapMin:40,restEvery:4,restMin:20},
  Realistic:{mph:65,fuelEvery:650,fuelMin:15,swapMin:35,restEvery:5,restMin:20},
  Push:{mph:68,fuelEvery:800,fuelMin:20,swapMin:30,restEvery:6,restMin:15}
};
export const PRESET_VERSION = 3;             // bump when PRESETS change, or saved copies override them
...
/* Solve arrival time. Swaps land on a fixed clock, so their count depends on the arrival
   time — which depends on how many swaps we took. Iterate to a fixed point; each pass only
   adds stops, so it converges (2-3 passes). Pure: no DOM, no shared state. */
export function solveEta({ miles, p, swap, startMs }){
  ...
  let totalH = driveH, swaps = [], restStops = 0, stopH = 0;
  for(let i=0;i<8;i++){
    const end = new Date(startMs + totalH*3600e3);
    swaps = swapTimes(start, end, swap.tz, swap.times);
    restStops = Math.max(0,
      Math.ceil(driveH/(Number(p.restEvery)||999)) - 1 - fuelStops - swaps.length);
    stopH = (swaps.length*(Number(p.swapMin)||0)
           + fuelStops*(Number(p.fuelMin)||0)
           + restStops*(Number(p.restMin)||0)) / 60;
    const next = driveH + stopH;
    if(Math.abs(next-totalH) < 1e-4){ totalH = next; break; }
    totalH = next;
  }
  const tunedEta = new Date(startMs + totalH*3600e3);
  const quickH = miles / RATE;
  const quickEta = new Date(startMs + quickH*3600e3);
  return { driveH, fuelStops, swaps, restStops, stopH, totalH, tunedEta, quickH, quickEta };
}
